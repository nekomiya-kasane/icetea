...
auto icu4x_bridge::set_locale(std::string_view locale, bool lazy) -> result<void> {
    if (locale.size() > max_locale_length) {
        return std::unexpected(error{
            ...
            std::string("ICU4X: locale tag too long (") + std::to_string(locale.size()) + " > " +
...
auto icu4x_bridge::ensure_initialized() const -> result<void> {
    ...
    if (locale_tag_.empty()) {
        return std::unexpected(error{
            error_code::invalid_argument, "ICU4X: no locale set"});
    }
    auto* raw = icu4x_bridge_create(locale_tag_.c_str());
    if (!raw) {
        return std::unexpected(error{
            error_code::icu4x_init_failed,
...
auto icu4x_bridge::current_locale() const -> std::string {
    if (handle_) {
        ...
        auto n = icu4x_get_locale(
            static_cast<const Icu4xBridge*>(handle_.get()),
...
auto icu4x_bridge::cardinal_category(double n) const -> plural_category {
    ...
    auto cat = icu4x_cardinal_category(
        static_cast<const Icu4xBridge*>(handle_.get()), n);
...
auto icu4x_bridge::ordinal_category(double n) const -> plural_category {
    if (auto r = ensure_initialized(); !r) return plural_category::other;
    auto cat = icu4x_ordinal_category(
        static_cast<const Icu4xBridge*>(handle_.get()), n);
    if (cat < 0 || cat > 5) return plural_category::other;
    return static_cast<plural_category>(cat);
}
...
void icu4x_bridge::format_number(std::string& out, double n, int fraction_digits) const {
    ...
    auto written = icu4x_format_number(
        static_cast<const Icu4xBridge*>(handle_.get()),
        n, fraction_digits, buf.data(), static_cast<int>(buf.size()));
    if (written < 0) {
        out.append(std::to_string(n));
        return;
    }
    if (written >= static_cast<int>(buf.size())) {
        ...
        icu4x_format_number(
            static_cast<const Icu4xBridge*>(handle_.get()),
            n, fraction_digits, big.data(), static_cast<int>(big.size()));
...
void ffi_format_into(std::string& out, std::string_view fallback,
                     const void* handle, Fn&& fn) {
    if (!handle) {
        out.append(fallback);
        return;
    }
    std::array<char, 256> buf{};
    auto written = fn(static_cast<const Icu4xBridge*>(handle),
                      buf.data(), static_cast<int>(buf.size()));
    if (written < 0) {
        out.append(fallback);
        return;
    ...
    if (written >= static_cast<int>(buf.size())) {
        std::string big(static_cast<size_t>(written) + 1, '\0');
        fn(static_cast<const Icu4xBridge*>(handle),
           big.data(), static_cast<int>(big.size()));
        out.append(big.data(), static_cast<size_t>(written));
        return;
    }
    out.append(buf.data(), static_cast<size_t>(written));
...
void icu4x_bridge::format_date(std::string& out, int year, int month, int day,
                                date_style style) const {
    ffi_format_into(out, std::to_string(year) + "-" + std::to_string(month) + "-" + std::to_string(day),
                    handle_.get(),
                    [&](const Icu4xBridge* b, char* buf, int len) {
                        return icu4x_format_date(b, year, month, day,
                                                 static_cast<int>(style), buf, len);
                    });
}

void icu4x_bridge::format_time(std::string& out, int hour, int minute, int second) const {
    auto fallback = std::to_string(hour) + ":" +
                    (minute < 10 ? "0" : "") + std::to_string(minute) + ":" +
                    (second < 10 ? "0" : "") + std::to_string(second);
    ffi_format_into(out, fallback, handle_.get(),
                    [&](const Icu4xBridge* b, char* buf, int len) {
...
void icu4x_bridge::format_list(std::string& out,
    ...
    auto written = icu4x_format_list(
        static_cast<const Icu4xBridge*>(handle_.get()),
        c_ptrs.data(), static_cast<int>(c_ptrs.size()),
        static_cast<int>(type), buf.data(), static_cast<int>(buf.size()));
    ...
    if (written >= static_cast<int>(buf.size())) {
        ...
        icu4x_format_list(
            static_cast<const Icu4xBridge*>(handle_.get()),
            c_ptrs.data(), static_cast<int>(c_ptrs.size()),
            static_cast<int>(type), big.data(), static_cast<int>(big.size()));
        out.append(big.data(), static_cast<size_t>(written));
        return;
    }
    out.append(buf.data(), static_cast<size_t>(written));
...
void icu4x_bridge::format_currency(std::string& out, double amount,
                                    std::string_view currency_code) const {
    std::string code_str(currency_code);
    ffi_format_into(out, std::to_string(amount) + " " + code_str,
                    handle_.get(),
                    [&](const Icu4xBridge* b, char* buf, int len) {
...
void icu4x_bridge::format_reltime(std::string& out, double value,
                                   reltime_unit unit) const {
    auto iv = static_cast<int>(value);
    auto fallback = iv < 0
        ? std::to_string(-iv) + " units ago"
        : "in " + std::to_string(iv) + " units";
    ffi_format_into(out, fallback, handle_.get(),
                    [&](const Icu4xBridge* b, char* buf, int len) {
...
auto icu4x_bridge::cardinal_range(double start, double end) const -> plural_category {
    if (auto r = ensure_initialized(); !r) return plural_category::other;
    auto cat = icu4x_cardinal_range(
        static_cast<const Icu4xBridge*>(handle_.get()), start, end);
    if (cat < 0 || cat > 5) return plural_category::other;
    return static_cast<plural_category>(cat);
}
