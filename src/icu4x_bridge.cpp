...
auto icu4x_bridge::ensure_initialized() const -> result<void> {
    ...
    if (!raw) {
        return std::unexpected(error{error_code::icu4x_init_failed,
...
void icu4x_bridge::format_number(std::string &out, double n, int fraction_digits) const {
    ...
    auto written = icu4x_format_number(static_cast<const Icu4xBridge *>(handle_.get()), n, fraction_digits, buf.data(),
...
template <typename Fn> void ffi_format_into(std::string &out, std::string_view fallback, const void *handle, Fn &&fn) {
    if (!handle) {
        out.append(fallback);
        return;
    }
...
void icu4x_bridge::format_date(std::string &out, int year, int month, int day, date_style style) const {
    ffi_format_into(out, std::to_string(year) + "-" + std::to_string(month) + "-" + std::to_string(day), handle_.get(),
